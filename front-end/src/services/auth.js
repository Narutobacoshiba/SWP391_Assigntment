...
export class AuthService {
  ...
  static async makeLogin ({ username, password }) {
    try {
      const response = await axios.post(`${API_URL}/SWP_391_Assignment/user/get`,{username, password}, {useCredentails: true})
      _setAuthData({
  ...
  static async makeRegister ({username, email, password }) {
    try {
      const response = await axios.post(`${API_URL}/SWP_391_Assignment/user/register`,
  ...
  static async refreshTokens () {
    try {
      ...
      const response = await axios.post(`${API_URL}/SWP_391_Assignment/jwt/refreshToken`,{ "id": currentUserId },{useCredentails: true})
      _setAuthData({
        accessToken: response.data.accessToken,
        exp: _parseTokenData(response.data.accessToken).exp
      })
